...
export function alterQuantity(item, new_quantity) {
  ...
  let cartItem = cart.find(it => it.Id == item.Id && it.SelectedColor == item.SelectedColor);
...
export function removeItem(item) {
  ...
  setLocalStorage("so-cart", cart.filter(it => it.Id != item.Id || it.SelectedColor != item.SelectedColor));
  ...
  if (cartItems != null) {
    ...
    for (let i = 0; i < buttons.length; i++) {
	  (function () {
	    ...
	    buttons[i].addEventListener("click", () => { removeItem(item); });
	    inputs[i].addEventListener("change", () => { alterQuantity(item, inputs[i].value); inputs[i].focus(); });
...
function renderCartItem(item) {
  ...
  <input type="number" name="cart-card__quantity-input" class="cart-card__quantity-input" value="${item.Quantity}" min="1" max="99">
  <p class="cart-card__price"><span class="struck-price">$${item.SuggestedRetailPrice.toFixed(2)}</span>&ensp;<span class="discount-price">$${item.FinalPrice.toFixed(2)}</span></p>
</li>`;
